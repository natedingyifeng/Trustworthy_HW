...
def main(cfg):
    ...
    for corruption_type in corruption_types:
        ...
        for tune_option in tune_options:
            ...
            if cfg.data.dataset_name == "cifar10" and tune_option == "last":
                lr_wd_grid = [
                    (1e-1, 1e-4),
                    (1e-2, 1e-4),
                    (1e-3, 1e-4),
            ...
            for lr, wd in lr_wd_grid:
                ...
                if cfg.data.dataset_name == "cifar10":
                    layer_to_block_dict = {
                        "conv1": "first_two_block",
                        "block1": "first_two_block",
                        "block2": "second_block",
                        "block3": "third_block",
                        "fc": "last",
                    }
                ...
                for epoch in range(1, cfg.args.epochs + 1):
                    ...
                    if cfg.args.auto_tune != 'none': 
                        if cfg.args.auto_tune == 'RGN':
                        ...
                        elif cfg.args.auto_tune == 'RGN-with-pass-ochiai':
                            weights_fail = get_lr_weights(model, loaders["train"], cfg)
                            max_weight_fail = max(weights_fail.values())
                            weights_pass = get_lr_weights(model, loaders["train_clean"], cfg)
                            max_weight_pass = max(weights_pass.values())
                            weights = {}
                            for k in weights_pass:
                                weights[k] = weights_fail[k] / (math.sqrt((weights_fail[k] + weights_pass[k])*max_weight_fail))
                            mean_weight = mean(weights.values())
                            mean_weight_fail = mean(weights_fail.values())
                            max_weight = max(weights.values())
                            for k, v in weights.items(): 
                                weights[k] = v / max_weight
                            layer_weights = [sum(x) for x in zip(layer_weights, weights.values())]
                            ...
                            for param, weight in weights.items():
                                params_weights.append({"params": params[param], "lr": weight*lr})
                            opt = optim.Adam(params_weights, lr=lr, weight_decay=wd)
                        elif cfg.args.auto_tune == 'RGN-with-pass-jaccard':
                            weights_fail = get_lr_weights(model, loaders["train"], cfg)
                            max_weight_fail = max(weights_fail.values())
                            weights_pass = get_lr_weights(model, loaders["train_clean"], cfg)
                            max_weight_pass = max(weights_pass.values())
                            weights = {}
                            for k in weights_pass:
                                weights[k] = weights_fail[k] / (weights_pass[k] + max_weight_fail)
                            mean_weight = mean(weights.values())
                            mean_weight_fail = mean(weights_fail.values())
                            max_weight = max(weights.values())
                            for k, v in weights.items(): 
                            ...
                            for param, weight in weights.items():
                                params_weights.append({"params": params[param], "lr": weight*lr})
                            opt = optim.Adam(params_weights, lr=lr, weight_decay=wd)
                        elif cfg.args.auto_tune == 'RGN-with-pass-wong2-version-1':
                            weights_fail = get_lr_weights(model, loaders["train"], cfg)
                            max_weight_fail = max(weights_fail.values())
                            weights_pass = get_lr_weights(model, loaders["train_clean"], cfg)
                            max_weight_pass = max(weights_pass.values())
                            weights = {}
                            for k in weights_pass:
                                weights[k] = abs(weights_fail[k] - weights_pass[k])
                            mean_weight = mean(weights.values())
                            mean_weight_fail = mean(weights_fail.values())
                            max_weight = max(weights.values())
                            if max_weight != 0:
                            ...
                            for param, weight in weights.items():
                                params_weights.append({"params": params[param], "lr": weight*lr})
                            opt = optim.Adam(params_weights, lr=lr, weight_decay=wd)
                        elif cfg.args.auto_tune == 'RGN-with-pass-wong2-version-2':
                            weights_fail = get_lr_weights(model, loaders["train"], cfg)
                            max_weight_fail = max(weights_fail.values())
                            weights_pass = get_lr_weights(model, loaders["train_clean"], cfg)
                            max_weight_pass = max(weights_pass.values())
                            weights = {}
                            for k in weights_pass:
                                weights[k] = max(weights_fail[k] - weights_pass[k], 0)
                            mean_weight = mean(weights.values())
                            mean_weight_fail = mean(weights_fail.values())
                            max_weight = max(weights.values())
                            if max_weight != 0:
                            ...
                            for param, weight in weights.items():
                                params_weights.append({"params": params[param], "lr": weight*lr})
                            opt = optim.Adam(params_weights, lr=lr, weight_decay=wd)
                        elif cfg.args.auto_tune == 'RGN-with-pass-tarantula':
                            weights_fail = get_lr_weights(model, loaders["train"], cfg)
                            max_weight_fail = max(weights_fail.values())
                            weights_pass = get_lr_weights(model, loaders["train_clean"], cfg)
                            max_weight_pass = max(weights_pass.values())
                            weights = {}
                            for k in weights_pass:
                                weights[k] = (weights_fail[k]/max_weight_fail) / (weights_pass[k]/max_weight_pass + weights_fail[k]/max_weight_fail)
                            mean_weight = mean(weights.values())
                            mean_weight_fail = mean(weights_fail.values())
                            max_weight = max(weights.values())
                            for k, v in weights.items(): 
